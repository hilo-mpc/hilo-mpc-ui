...
export function MpcNode({ id, data, selected }: NodeProps<MpcBlockData>) {
  ...
  return (
    ...
    >
      ...
      {/* Handles — left: model (top) + measurement (bottom); right: control (top) + results (bottom) */}
      <Handle
        type="target"
        position={L}
        id="mpc-model-in"
      ...
      <Handle
        ...
        id="mpc-results-out"
        style={{ top: '70%' }}
        className="!w-3 !h-3 !bg-violet-300 !border-2 !border-stone-800"
        title="Results for Plot"
      />
    </div>
  );
}
