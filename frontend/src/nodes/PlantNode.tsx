...
export function PlantNode({ id, data, selected }: NodeProps<PlantBlockData>) {
  ...
  return (
    <div className="relative min-w-[160px]">
      ...
      {/* Handles */}
      <Handle type="target" position={L} id="plant-control-in"
        className="!w-3 !h-3 !bg-teal-400 !border-2 !border-stone-800" />
      <Handle type="source" position={R} id="plant-measurement-out"
        style={{ top: '35%' }} className="!w-3 !h-3 !bg-violet-400 !border-2 !border-stone-800" />
      <Handle type="source" position={R} id="plant-states-out"
        style={{ top: '70%' }} className="!w-3 !h-3 !bg-orange-400 !border-2 !border-stone-800" />

      {/* Tags */}
      <HandleTag nodeId={id} handleId="plant-control-in"      label="MPC"       side={sideL} />
      <HandleTag nodeId={id} handleId="plant-measurement-out" label="MPC · MHE" side={sideR} top="35%" />
      <HandleTag nodeId={id} handleId="plant-states-out"      label="Plot"      side={sideR} top="70%" />
    </div>
  );
}
