...
export function MheNode({ id, data: _data, selected }: NodeProps) {
  ...
  return (
    <div className="relative min-w-[170px]">
      ...
      {/* Handles */}
      <Handle type="target" position={L} id="mhe-model-in"
        style={{ top: '33%' }} className="!w-3 !h-3 !bg-rose-400 !border-2 !border-stone-800" />
      <Handle type="target" position={L} id="mhe-data-in"
        style={{ top: '67%' }} className="!w-3 !h-3 !bg-sky-400 !border-2 !border-stone-800" />
      <Handle type="source" position={R} id="mhe-states-out"
        className="!w-3 !h-3 !bg-fuchsia-300 !border-2 !border-stone-800" />

      {/* Tags */}
      <HandleTag nodeId={id} handleId="mhe-model-in"   label="Model"        side={sideL} top="33%" />
      <HandleTag nodeId={id} handleId="mhe-data-in"    label="Data · Plant" side={sideL} top="67%" />
      <HandleTag nodeId={id} handleId="mhe-states-out" label="MPC · Plot"   side={sideR} />
    </div>
  );
}
