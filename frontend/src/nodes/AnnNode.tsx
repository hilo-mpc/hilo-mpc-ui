...
export function AnnNode({ id, data: _data, selected }: NodeProps) {
  ...
  const trainHandleTop = hasTrained ? '35%' : '50%';
  const resultsHandleTop = hasTrained ? '35%' : '50%';
  ...
  return (
    <div className="relative min-w-[180px]">
            ...
            <>
              ...
              {hasTrained && (
                <button
                  ...
                  title="Run prediction on connected data"
                >
                  Predict
                </button>
              )}
      ...
      <Handle type="target" position={L} id="ann-data-in"
        style={{ top: trainHandleTop }} className="!w-3 !h-3 !bg-sky-400 !border-2 !border-stone-800" />
      {hasTrained && (
        <Handle type="target" position={L} id="fn-input"
          style={{ top: '70%' }} className="!w-3 !h-3 !bg-sky-300 !border-2 !border-stone-800" />
      )}
      <Handle type="source" position={R} id="ann-results-out"
        style={{ top: resultsHandleTop }} className="!w-3 !h-3 !bg-indigo-300 !border-2 !border-stone-800" />
      {hasTrained && (
        <Handle type="source" position={R} id="fn-output"
          style={{ top: '70%' }} className="!w-3 !h-3 !bg-emerald-400 !border-2 !border-stone-800" />
      )}

      {/* Tags */}
      <HandleTag nodeId={id} handleId="ann-data-in"    label="Data" side={sideL} top={trainHandleTop} />
      {hasTrained && (
        <HandleTag nodeId={id} handleId="fn-input"     label="Data" side={sideL} top="70%" />
      )}
      <HandleTag nodeId={id} handleId="ann-results-out" label="Plot" side={sideR} top={resultsHandleTop} />
      {hasTrained && (
        <HandleTag nodeId={id} handleId="fn-output"    label="Plot" side={sideR} top="70%" />
      )}
    </div>
  );
}
