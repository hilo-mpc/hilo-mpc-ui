...
export function PlotNode({ id, data: _data, selected }: NodeProps) {
  ...
  const sideIn = data.flipped ? 'right' : 'left';
  ...
  return (
    <div className="relative min-w-[220px]" style={{ width: 340 }}>
      <div
      ...
      />
      <HandleTag nodeId={id} handleId="plot-data-in" label="Results" side={sideIn} />
    </div>
  );
}
