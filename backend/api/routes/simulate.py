...
_runs: dict[str, tuple[SimulationResult, SimulationRequest]] = {}


@router.post("/simulate")
async def start_simulation(req: SimulationRequest):
    run_id = str(uuid.uuid4())
    result = SimulationResult(run_id=run_id, status="running")
    _runs[run_id] = (result, req)
    return {"run_id": run_id}


@router.delete("/simulate/{run_id}", status_code=204)
...
async def simulate_ws(websocket: WebSocket, run_id: str):
    ...
    try:
        await run_simulation(req, send_frame)
